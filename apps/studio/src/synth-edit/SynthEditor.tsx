...
function SynthEditorInner() {
  ...
  const handleLoadSample = useCallback(async () => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        if (meta) setSampleDuration(`${meta.duration.toFixed(2)}s (${Math.round(meta.duration * meta.sampleRate)} samples @ ${meta.sampleRate}Hz)`);
        const data = SampleCache.get(sampleId);
        if (data) PolySynthOutput.loadSample(sampleId, data, meta?.sampleRate ?? 44100);
  ...
  const handleSavePreset = useCallback(() => {
    ...
    if (name) instrumentManager.saveCustomPreset(name, isSampler ? "sampler" : "synth", localConfig.sampleId as string);
  ...
  function renderPianoKey(noteIndex: number, isBlack: boolean, label: string) {
    ...
    const style: React.CSSProperties = isBlack
      ? { position: "absolute", left: `${(noteIndex / 12) * 100}%`, width: "7%", height: "60%", top: 0, zIndex: 1, background: "#222", borderRadius: "0 0 3px 3px", cursor: "pointer", border: "1px solid #444" }
      : { width: `${100 / 7}%`, height: "100%", background: "#ddd", border: "1px solid #aaa", borderRadius: "0 0 4px 4px", cursor: "pointer", color: "#333", fontSize: "8px", display: "flex", alignItems: "flex-end", justifyContent: "center", paddingBottom: "2px" };
    return (
      <div
        key={midiNote}
        style={style}
        onMouseDown={(e) => { e.preventDefault(); keyNoteOn(midiNote); }}
  ...
  return (
    <div className="flex flex-col h-full text-[11px] text-[#ccc] overflow-y-auto">
      ...
      <div className="flex items-center gap-2 p-2 border-b border-[#4a4a4a] shrink-0">
        ...
        <button className="px-2 py-1 bg-[#4a4a4a] rounded text-[10px] hover:bg-[#555]" onClick={handleSavePreset}>Save</button>
      </div>
        ...
        <div className="p-2 space-y-3 border-b border-[#4a4a4a] shrink-0">
          ...
          {sampleFile && (
            <div className="text-[10px] text-[#999] space-y-0.5">
              <div><span className="text-[#888]">File:</span> {sampleFile}</div>
              {sampleDuration && <div><span className="text-[#888]">Info:</span> {sampleDuration}</div>}
            </div>
          )}
          <div>
            <label className="text-[10px] text-[#888] block mb-1">Root Note</label>
            <select
              className="w-full bg-[#3a3a3a] border border-[#555] rounded px-2 py-1 text-[11px]"
              value={(localConfig.rootNote as number) ?? 60}
              onChange={(e) => updateConfig({ rootNote: Number(e.target.value) })}
            >
              {Array.from({ length: 61 }, (_, i) => i + 24).map((note) => {
                const oct = Math.floor(note / 12) - 1;
                const name = NOTES[note % 12];
                return <option key={note} value={note}>{name}{oct} ({note})</option>;
              })}
            </select>
          </div>
        </div>
      ...
      {!isSampler && (
        <div className="p-2 space-y-3 border-b border-[#4a4a4a] shrink-0">
          <div>
            <label className="text-[10px] text-[#888] block mb-1">Oscillator Type</label>
            <select
              className="w-full bg-[#3a3a3a] border border-[#555] rounded px-2 py-1 text-[11px]"
              value={localConfig.oscillatorType as string ?? "saw"}
              onChange={(e) => updateConfig({ oscillatorType: e.target.value })}
            >
              {["saw", "square", "sine", "triangle", "noise", "fm", "pluck"].map((t) => (
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              { label: "Detune", key: "oscillatorDetune", min: -100, max: 100, step: 1 },
              { label: "Cutoff", key: "filterCutoff", min: 20, max: 20000, step: 10 },
              { label: "Resonance", key: "filterResonance", min: 0, max: 1, step: 0.01 },
              { label: "Volume", key: "volume", min: 0, max: 1, step: 0.01 },
            ].map(({ label, key, min, max, step }) => (
              <div key={key}>
                <label className="text-[9px] text-[#888] block text-center">{label}</label>
                <input
                  type="range"
      ...
      <div className="p-2 space-y-2 border-b border-[#4a4a4a] shrink-0">
        <div className="text-[10px] text-[#888] uppercase tracking-wider mb-1">ADSR</div>
        <div className="grid grid-cols-4 gap-2">
          {[
      ...
      <div className="mt-auto p-2 shrink-0">
        <div className="relative h-16 flex">
          {WHITE_KEYS.map((ni) => renderPianoKey(ni, false, `${NOTES[ni]}${octaveOffset}`))}
          {BLACK_KEYS.map((ni) => renderPianoKey(ni, true, `${NOTES[ni]}${octaveOffset}`))}
        </div>
      </div>
    </div>
  );
