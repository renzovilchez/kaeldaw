...
export function FloatingWindow({ id, children, sidebarWidth = 0, onFocus }: Props) {
  ...
  const handleMouseDown = useCallback((e: MouseEvent) => {
    ...
    dragRef.current = { startX: e.clientX, startY: e.clientY, winX: w.position.x, winY: w.position.y, winW: w.size.width, winH: w.size.height };

    const onMove = (ev: globalThis.MouseEvent) => {
      if (!dragRef.current) return;
      const dx = ev.clientX - dragRef.current.startX;
      const dy = ev.clientY - dragRef.current.startY;
      const newX = Math.max(sidebarWidthRef.current + 1, Math.min(window.innerWidth - dragRef.current.winW, dragRef.current.winX + dx));
      const newY = Math.max(41, Math.min(window.innerHeight - dragRef.current.winH, dragRef.current.winY + dy));
      move(id, newX, newY);
    };
    const onUp = () => {
  ...
  const startResize = useCallback((e: MouseEvent) => {
    ...
    const onMove = (ev: globalThis.MouseEvent) => {
      ...
      resize(id, Math.min(maxW, Math.max(200, startW + dw)), Math.min(maxH, Math.max(100, startH + dh)));
    };
    const onUp = () => {
      setIsResizing(false);
      document.removeEventListener("mousemove", onMove);
  ...
  const style: React.CSSProperties = w.isMaximized
    ? { position: "fixed", inset: `41px 0 0 ${sidebarWidth + 1}px`, zIndex: w.zIndex, display: "flex", flexDirection: "column" }
    : { position: "fixed", left: w.position.x, top: w.position.y, width: w.size.width, height: w.size.height, zIndex: w.zIndex, display: "flex", flexDirection: "column" };
  ...
  return (
    ...
    >
      <div
        ref={titleRef}
        className="h-7 bg-gradient-to-r from-[#4a4a4a] to-[#555] flex items-center justify-between px-2 cursor-grab active:cursor-grabbing shrink-0 select-none"
        onMouseDown={handleMouseDown}
        onDoubleClick={handleDoubleClick}
      >
        <span className="text-[11px] text-[#ccc] font-medium tracking-wide">{w.title}</span>
      ...
      {!w.isMaximized && (
        ...
        >
          <svg viewBox="0 0 8 8" className="w-3 h-3 absolute bottom-0.5 right-0.5" fill="#666">
